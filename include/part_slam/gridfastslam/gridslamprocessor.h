...
{
public:
    // 树的节点，一个树存了以整条轨迹，一个节点表示这条轨迹中的其中一个点。存储激光雷达的整条轨迹
    // 一个节点（TNode）包含了：
    struct TNode
    {
        TNode(const OrientedPoint& pose, TNode* parent=0);      // 构造函数
        ~TNode();                                               // 析构函数

        OrientedPoint pose;                 // 该节点粒子的激光雷达位姿
        const RangeReading* reading;        // 该节点激光雷达的读数
        TNode* parent;                      // 指向父节点的指针
    };
    ...
    // 设置扫描匹配的参数
    void setMatchingParameters(double urange, double range, double sigma, int kernel_size,
                               double l_opt,  double a_opt, int iterations, double likelihoodSigma=1,
    ...
    /*
     * 设置地图尺寸，分辨率，初始位姿，清空给每个粒子
     * new 一个轨迹树的根节点，初始化，初始位姿
     * 定义一个地图，为size个粒子，初始化，     地图、位姿、权重、轨迹
     * */
    void init(unsigned int size, double x_min, double y_min, double x_max, double y_max, double delta,
    ...
    // 获得粒子的数量
    inline const ParticleVector& getParticles() const { return m_particles_; }
    // 获得最优位姿粒子的序号
    ...
    // 这里是运行模型中涉及的四个方差：
    STRUCT_PARAM_SET_GET(m_motionModel_, double, srr, protected, public, public);       // 线性运动造成的线性误差的方差
    STRUCT_PARAM_SET_GET(m_motionModel_, double, srt, protected, public, public);       // 线性运动造成的角度误差的方差
    STRUCT_PARAM_SET_GET(m_motionModel_, double, str, protected, public, public);       // 旋转运动造成的线性误差的方差
    STRUCT_PARAM_SET_GET(m_motionModel_, double, stt, protected, public, public);       // 旋转运动造成的角度误差的方差
...
protected:
    ...
    double                      period_;
    // 激光束的数量
    unsigned int                m_beams_;
    // 粒子数组
    ParticleVector              m_particles_;
    // 重采样之后，剩余的粒子的下标，这个下标是会重复的
    std::vector<unsigned int>   m_indexes_;
    // 所有粒子的当前的权重
    std::vector<double>         m_weights_;
    // 粒子运动模型
    MotionModel                 m_motionModel_;
    // 上一次的位姿
    OrientedPoint               m_odoPose_;
    // 激光雷达的位姿
    OrientedPoint               m_pose_;

    // 被处理过的激光雷达帧数
    int                         m_count_;
    // 记录两帧之间激光雷达直线位移
    double                      m_linearDistance_;
    // 记录两帧之间激光雷达角度位移
    double                      m_angularDistance_;

    // 定义一大堆变量以及其set和get函数
    // 地图尺寸，精度
    PARAM_GET(double, x_min, protected, public);
    PARAM_GET(double, y_min, protected, public);
    PARAM_GET(double, x_max, protected, public);
    PARAM_GET(double, y_max, protected, public);
...
private:
     ...
     * 通过爬山函数，每一层向 前、后、左、右、左转、右转，6个方向，为每个粒子寻找最优位姿，直到最优位姿分数开始下降，或者超过迭代次数，停止爬山
     * 关于位姿得分，由于score函数计算，在击中栅格周围的9个栅格，寻找最优得分
     * */
    ...
    // 粒子重采样  根据neff(离散程度)的大小来进行重采样   不但进行了重采样，也对地图进行更新
...
inline void GridSlamProcessor::scanMatch(const double* plainReading)
{
    ...
    for (int i=0; i<particle_number; i ++)
    {
        ...
        // 输入当前地图和当前最优位姿，遍历激光束累计似然，把累计似然当作该粒子的权重，误差越大，似然越大
        // 似然大小，代表权重大小
        m_matcher.likelihoodAndScore(l, m_particles_[i].map, m_particles_[i].pose, plainReading);
        // 为每个粒子更新权重和累计权重
        m_particles_[i].weight += l;        // 该粒子的权重
        m_particles_[i].weightSum += l;     // 该粒子的权重累计，寻找最优粒子使用
    }
}
...
inline bool GridSlamProcessor::resample(const double* plainReading, int adaptSize, const RangeReading* reading)
{
    ...
    {
        oldGeneration.push_back(m_particles_[i].node);
    }

    // 如果需要进行重采样
    if (m_neff < m_resampleThreshold * m_particles_.size())
    {
    ...
    else
    {
        ...
        int particle_size = m_particles_.size();
        //为每个粒子更新地图
        for(int i = 0; i < particle_size;i++)
        {
            //创建一个新的树节点
            TNode* node = 0;
            node = new TNode(m_particles_[i].pose,oldGeneration[i]);

            //把这个节点接入到树中
            node->reading = reading;
            m_particles_[i].node = node;//每一个粒子都指向一个最新的节点

            //拓展地图大小、找到地图的有效区域，单位patch,申请内存、更新每个栅格的内容
            m_matcher.computeMap(m_particles_[i].map, m_particles_[i].pose, plainReading);
        }
