...
{                                                                                   // tf_是激光雷达坐标系基准坐标系到里程计坐标系的变换
    ...
    double start_time = startTime.toSec() * 1000 * 1000;        // *1000*1000 转化时间单位为us
    ...
    // 当前插值的段的起始坐标
    int start_index = 0;
    // 默认基准坐标系就是第一个位姿的坐标系                                         这是后面进行 点云去畸变的 基准坐标
...
bool LidarMotionCalibrator::getLaserPose(tf::Stamped<tf::Pose>& odom_pose, ros::Time dt, tf::TransformListener* tf_)
{
    ...
    try
    {
        ...
        // 转换一个 带时间戳的位姿 到 目标坐标系odom_name_的位姿 输出到 odom_pose
...
{                                                                                   // 起始插值索引      插值的个数
    ...
    tf::Vector3  base_pos = frame_base_pose.getOrigin();    base_pos.setZ(0);     // 因为是平面，所以这里Z轴数据设置为0
